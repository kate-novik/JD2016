...
public final class Add {
    ...
    public static Vector add(Vector var1, Vector var2) {
        if (var1.getSize() == var2.getSize()) {
            ...
            try {
                throw new ArithmeticException("Операция невозможна! Вектора различны по размерности.");
            } catch (ArithmeticException e) {
                Logger log = Logger.getInstance();
                log.logError(e);
            }
    ...
    public static Matrix add(Matrix var1, Matrix var2) {
        if (var1.getRow() == var2.getRow() && var1.getCol() == var2.getCol()) {
            ...
            try {
                throw new ArithmeticException("Операция невозможна! Матрицы несоразмерны.");
            } catch (ArithmeticException e) {
                Logger log = Logger.getInstance();
                log.logError(e);
            }
