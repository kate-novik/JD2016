...
import by.it.luksha.jd02_06.log.Logger;
...
public final class Mult {
    ...
    public static Scalar mult(Vector var1, Vector var2) {
        if (var1.getSize() == var2.getSize()) {
            ...
            try {
                throw new ArithmeticException("Операция невозможна! Вектора несоразмерны.");
    ...
    public static Vector mult(Matrix var2, Vector var1) {
        if (var1.getSize() == var2.getCol()) {
            ...
            try {
                throw new ArithmeticException("Операция невозможна! Вектор-столбец и матрица несоразмерны.");
            } catch (ArithmeticException e) {
                Logger log = Logger.getInstance();
                log.logError(e);
            }
    ...
    public static Matrix mult(Matrix var1, Matrix var2) {
        if (var1.getCol() == var2.getRow()) {
            ...
            try {
                throw new ArithmeticException("Операция невозможна! Кол-во столбцов первого множителя не равно кол-ву строк второго множителя.");
            } catch (ArithmeticException e) {
                Logger log = Logger.getInstance();
                log.logError(e);
            }
